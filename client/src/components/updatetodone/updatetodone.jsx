...
import './updatetodone.css'

function updatetodone({isOpen,onClose,orderId,orderItem}) {
                  ...
                  {orders.map(item => (<>
                    <div className="update-content"  key={item.order_items_id}>
                        ...
                        <div className="order-update-action">
                            ...
                            {/* <button className="button-left" >ดาวน์โหลดดราฟงาน</button>  */}
                            <a href={`http://localhost:2000/images/` + item.task_img} download={item.order_items_id}>
                              <button className="button-right" >Download File</button>
                            </a>
                            <button className="button-right" onClick={() => handleClick('4', '5')}>ยอมรับคำสั่งซื้อ</button>
                        </div>
                    </div>
                  </>))}</>):null}
                {/* order-box end here */}
