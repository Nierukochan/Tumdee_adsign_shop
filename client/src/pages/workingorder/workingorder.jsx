...
import Updatetodone from '../../components/updatetodone/updatetodone'
...
function workingorder() {
  ...
  useEffect(() => {
    const fetchItems = async () => {
      const response = await axios.get('http://localhost:2000/api/order/getorderworking', { withCredentials: true })
        ...
        <div className="content-container">
          ...
          <div className="order-manage-box">
              <div className="order-manage-head">
                <h1>คำสั่งซื้อที่กำลังดำเนินการ</h1>
              </div>
            ...
            {order.length > 0 ? (
              <>
                {order.map(item => (
                  <>
                  <div className="order-manage-map" key={item.order_items_id} onClick={() => handleUploadClick(item.order_id,item.order_items_id)}> {/* onClick={() => handleUploadClick(item.order_id,item.order_items_id) */}
